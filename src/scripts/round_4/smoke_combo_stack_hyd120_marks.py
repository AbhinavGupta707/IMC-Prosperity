"""Local smoke test for the combined Mark probe.

Replays days 1-3 against:
- expstack8060 base (current top, +9,318)
- expstack4060 base (+8,812)
- combo_stack_hyd120_marks_v1 (new combined probe)
"""
...
def main() -> None:
    ...
    files = [
        ("expstack8060", REPO_ROOT / "outputs/submissions/r4/submission_r4_exp_stack_hydhardlong80_60k.py"),
        ("combo_hyd120_nomark", REPO_ROOT / "outputs/submissions/r4/submission_r4_combo_stack_hyd120_nomark.py"),
        ("combo_hyd120_marks_v1", REPO_ROOT / "outputs/submissions/r4/submission_r4_combo_stack_hyd120_marks_v1.py"),
    ]
    ...
    for label, path in files:
        try:
            res = replay_one(path, f"r4_combo_smoke_{label}", data_dir, args.days)
        ...
        for prod in (
            ...
            if not v:
                continue
            print(f"  {prod}: pnl={v['pnl']:.2f} pos={v['final_position']:>4} qty={v['qty']}")

    if len(results) >= 2:
        base = results[0][1]["total_pnl"]
        print("\n=== Deltas vs expstack8060 ===")
        for label, res in results[1:]:
            ...
            for prod in (
                ...
                if pv and bv:
                    ...
                    print(f"    {prod}: pnl_delta={pdiff:+.2f}, pos={pv.get('final_position', 'n/a')}, qty_delta={pv['qty']-bv['qty']:+d}")
