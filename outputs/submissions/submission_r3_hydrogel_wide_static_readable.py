...
"""HYDROGEL_PACK wide static mean-reversion.

V7's aggressive profit-taking crossed spread near the old 9990 anchor and
created churn on the official path. The stronger HYDROGEL result from the
asset-level sweep was a wider, lower static-fair mean-reversion shape:

  - fair = 9980 + small inventory skew
  - cross only on wider deviations (take_width=30)
  - no forced flatten near the anchor

This keeps HYDROGEL as an asset-specific alpha sleeve while VELVET/options
remain controlled elsewhere in the R3 engine.
"""



# Static HYDROGEL fair chosen from HYDROGEL-only confirmation replay.
_HYDROGEL_MEAN: float = 9980.0
...
def hydrogel_orders(
    ...
    cap = scaled_cap(_HYDROGEL_POS_LIMIT, timestamp)
    fair_value = _anchored_fair(position, cap, skew_strength)
