...
_HYDROGEL_MEAN=9955.
_HYDROGEL_CYCLE_RESET_GAP=14.
_HYDROGEL_REBOUND_LONG_SIZE=40
_HYDROGEL_REBOUND_EXIT_GAP=35.
_HYDROGEL_REBOUND_COOLDOWN=10
_HYDROGEL_PARAMS=SSTParams(take_width=25.,clear_threshold=1.,clear_width=2.,default_edge=3.,disregard_edge=1.,join_edge=3.,default_quote_size=20,max_taker_size=200,prevent_adverse=False,quote_inside_wall=False,wall_min_volume=10)
...
def hydrogel_orders(snapshot,position,timestamp,params=_HYDROGEL_PARAMS,skew_strength=1.,cycle_state=None):
	...
	cycle_state=cycle_state if cycle_state is not None else{};rebound_exit=_rebound_long_exit_order(snapshot,position,cycle_state)
	if rebound_exit is not None:return[rebound_exit]
	reset_order=_cycle_reset_order(snapshot,position,cycle_state)
	...
	cap=scaled_cap(_HYDROGEL_POS_LIMIT,timestamp);fair_value=_anchored_fair(position,cap,skew_strength);decision=take_clear_make(product=HYDROGEL_PACK,fair_value=fair_value,snapshot=snapshot,position=position,position_limit=cap,params=params);orders=list(decision.orders);cooldown_left=int(cycle_state.get('cooldown_left',0)or 0)
	...
	return orders
def _cycle_reset_order(snapshot,position,cycle_state):
	if snapshot.mid is None:return
	mid=float(snapshot.mid)
	if position<0 and mid<=_HYDROGEL_MEAN-_HYDROGEL_CYCLE_RESET_GAP and snapshot.best_ask is not None:cycle_state['last_short_reset_mid']=mid;cycle_state['long_mode']=True;return Order(HYDROGEL_PACK,snapshot.best_ask.price,-position)
...
def _rebound_long_exit_order(snapshot,position,cycle_state):
	...
	mid=float(snapshot.mid)
	if mid<_HYDROGEL_MEAN+_HYDROGEL_REBOUND_EXIT_GAP:return
...
_HYDROGEL_REBOUND_TRADER=Trader
_VELVET_FLATTEN_START=980000
_VELVET_SCHEDULES={
	'VELVETFRUIT_EXTRACT':((0,{'limit':200,'max_order':80,'buy':5245,'sell':5269}),(100000,{'limit':200,'max_order':40,'buy':5246,'sell':5272})),
	'VEV_4000':((100000,{'limit':300,'max_order':10,'buy':1233,'sell':1263}),),
	'VEV_4500':((100000,{'limit':300,'max_order':20,'buy':732,'sell':766}),),
	'VEV_5000':((0,{'limit':300,'max_order':40,'buy':255,'sell':270}),(100000,{'limit':300,'max_order':20,'buy':241,'sell':273})),
	'VEV_5100':((0,{'limit':300,'max_order':40,'buy':165,'sell':179}),(100000,{'limit':300,'max_order':40,'buy':164,'sell':183})),
	'VEV_5200':((0,{'limit':300,'max_order':40,'buy':93,'sell':105}),(100000,{'limit':300,'max_order':40,'buy':93,'sell':105})),
	'VEV_5300':((0,{'limit':300,'max_order':20,'buy':45,'sell':52}),(100000,{'limit':300,'max_order':40,'buy':45,'sell':52})),
	'VEV_5400':((0,{'limit':300,'max_order':40,'buy':13,'sell':17}),(100000,{'limit':300,'max_order':40,'buy':15,'sell':18})),
	'VEV_5500':((0,{'limit':300,'max_order':40,'buy':5,'sell':7}),(100000,{'limit':300,'max_order':40,'buy':7,'sell':8})),
}
...
def _velvet_orders_for_product(product,schedule,state):
	...
	if state.timestamp<schedule[0][0]:return[]
	...
	return orders
class Trader:
	def __init__(self):self._hydrogel=_HYDROGEL_REBOUND_TRADER()
	def run(self,state):
