...
_HYDROGEL_MEAN=999e1
_PROFIT_TAKE_BAND=1e1
_PROFIT_TAKE_MIN_POS=30
_HYDROGEL_PARAMS=SSTParams(take_width=25.,clear_threshold=.9,clear_width=2.,default_edge=3.,disregard_edge=1.,join_edge=3.,default_quote_size=20,max_taker_size=200,prevent_adverse=False,quote_inside_wall=False,wall_min_volume=10)
...
def hydrogel_orders(snapshot,position,timestamp,params=_HYDROGEL_PARAMS,skew_strength=1.):
	...
	if abs(mid-_HYDROGEL_MEAN)<_PROFIT_TAKE_BAND and abs(position)>=_PROFIT_TAKE_MIN_POS:return _profit_take_orders(snapshot,position)
...
def _profit_take_orders(snapshot,position):
	if position>0 and snapshot.best_bid is not None:return[Order(HYDROGEL_PACK,snapshot.best_bid.price,-position)]
	if position<0 and snapshot.best_ask is not None:return[Order(HYDROGEL_PACK,snapshot.best_ask.price,-position)]
	return[]
