...
# ====================================================================
# R4 HYDROGEL PROBE — high-regime hard-long40 until bid 10052 or 70k.
#
# Calibration question:
#   Is the missing R4 official HYD alpha the rising-leg inventory that the
#   current R3-derived static short never owns?
#
# Mechanism:
#   - If HYD mid >= 10020 during 20k-30k, high-regime mode activates.
#   - Before release, replace HYD orders with a single order toward +40.
#   - Release when best bid >= 10052, or fall back at 70k.
#   - After release, let the base flat995 HYD sleeve trade normally.
# ====================================================================
_R4_HYD_HARD_BASE_TRADER = Trader
_HYD_HARD_TRIGGER_START = 20_000
_HYD_HARD_TRIGGER_END = 30_000
_HYD_HARD_TRIGGER_MID = 10_020.0
_HYD_HARD_FALLBACK_RELEASE_TS = 70_000
_HYD_HARD_RELEASE_BID = 10_052
_HYD_HARD_TARGET_POS = 40

class Trader:
    def __init__(self):
        self._inner = _R4_HYD_HARD_BASE_TRADER()
        self._hyd_high_regime = False
        self._hyd_released = False
    def run(self, state):
        orders, conversions, trader_data = self._inner.run(state)
        self._observe_hyd_regime(state)
        self._observe_hyd_release(state)
        if self._hyd_high_regime and not self._hyd_released:
            orders = self._hard_target_hyd(state, orders, _HYD_HARD_TARGET_POS)
        return orders, conversions, trader_data
    def _observe_hyd_regime(self, state):
        if self._hyd_high_regime:
            return
        if state.timestamp < _HYD_HARD_TRIGGER_START or state.timestamp > _HYD_HARD_TRIGGER_END:
            return
        depth = state.order_depths.get('HYDROGEL_PACK')
        if depth is None or not depth.buy_orders or not depth.sell_orders:
            return
        mid = (max(depth.buy_orders.keys()) + min(depth.sell_orders.keys())) / 2
        if mid >= _HYD_HARD_TRIGGER_MID:
            self._hyd_high_regime = True
    def _observe_hyd_release(self, state):
        if not self._hyd_high_regime or self._hyd_released:
            return
        if state.timestamp >= _HYD_HARD_FALLBACK_RELEASE_TS:
            self._hyd_released = True
            return
        depth = state.order_depths.get('HYDROGEL_PACK')
        if depth is not None and depth.buy_orders and max(depth.buy_orders.keys()) >= _HYD_HARD_RELEASE_BID:
            self._hyd_released = True
