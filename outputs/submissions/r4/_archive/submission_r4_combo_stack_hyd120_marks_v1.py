...
_R4_ALPHA_ENGINE_KEY='r4_stacked_alpha'
_R4_GATE_TS=30000
_R4_GATE_DROP=20.
_R4_VELVET_GATE_BUY=5248
_R4_VELVET_GATE_SELL=5264
_R4_CORE_RECYCLE_PRODUCTS=('VEV_5000','VEV_5100','VEV_5200')
_R4_CORE_RECYCLE_START=50000
_R4_CORE_RECYCLE_ACTIVE_ABS=280
_R4_CORE_RECYCLE_FLOOR_ABS=240
_R4_CORE_RECYCLE_TAKE_PROFIT=8.
_R4_CORE_RECYCLE_MAX_ORDER=20
_R4_CORE_RECYCLE_FALLBACK_SELL={'VEV_5000':263,'VEV_5100':170,'VEV_5200':95}
def _r4_default_alpha_state():
	return{'open_mid':None,'gate_decided':False,'gate_active':False,'last_trade_ts':-1,'last_seen_ts':-1,'costs':{}}
def _r4_load_alpha_state(trader_data):
	alpha=_r4_default_alpha_state()
	try:raw=json.loads(trader_data)if trader_data else{}
	except(TypeError,ValueError):return alpha
	if not isinstance(raw,dict):return alpha
	blob=(raw.get('engines')or{}).get(_R4_ALPHA_ENGINE_KEY,{})
	if not isinstance(blob,dict):return alpha
	if blob.get('open_mid')is not None:
		try:alpha['open_mid']=float(blob.get('open_mid'))
		except(TypeError,ValueError):alpha['open_mid']=None
	alpha['gate_decided']=bool(blob.get('gate_decided',False));alpha['gate_active']=bool(blob.get('gate_active',False))
	try:alpha['last_trade_ts']=int(blob.get('last_trade_ts',-1))
	except(TypeError,ValueError):alpha['last_trade_ts']=-1
	try:alpha['last_seen_ts']=int(blob.get('last_seen_ts',-1))
	except(TypeError,ValueError):alpha['last_seen_ts']=-1
	costs={}
	for product,payload in (blob.get('costs')or{}).items():
		if product not in _R4_CORE_RECYCLE_PRODUCTS or not isinstance(payload,dict):continue
		try:pos=int(payload.get('position',0))
		except(TypeError,ValueError):pos=0
		avg=payload.get('avg_price')
		try:avg=None if avg is None else float(avg)
		except(TypeError,ValueError):avg=None
		costs[product]={'position':pos,'avg_price':avg}
	alpha['costs']=costs
	return alpha
def _r4_save_alpha_state(trader_data,alpha):
	try:raw=json.loads(trader_data)if trader_data else{}
	except(TypeError,ValueError):raw={}
	if not isinstance(raw,dict):raw={}
	if not isinstance(raw.get('products'),dict):raw['products']={}
	if not isinstance(raw.get('engines'),dict):raw['engines']={}
	raw['version']=raw.get('version',1);raw['engines'][_R4_ALPHA_ENGINE_KEY]=alpha
	try:encoded=json.dumps(raw,separators=(',',':'),sort_keys=True,allow_nan=False)
	except(ValueError,TypeError):return trader_data
	return encoded if len(encoded)<=50000 else trader_data
def _r4_cost_apply(cost,side,price,qty):
	if qty<=0:return
	signed=qty if side=='buy'else-qty;pos=int(cost.get('position',0));avg=cost.get('avg_price')
	if pos==0:
		cost['position']=signed;cost['avg_price']=float(price);return
	if pos*signed>0:
		total=abs(pos)+qty;prev=float(avg)if avg is not None else float(price);cost['avg_price']=(prev*abs(pos)+float(price)*qty)/total;cost['position']=pos+signed;return
	remaining=pos+signed
	if pos*remaining>0:
		cost['position']=remaining;return
	if remaining==0:
		cost['position']=0;cost['avg_price']=None;return
	cost['position']=remaining;cost['avg_price']=float(price)
def _r4_process_own_trades(state,alpha):
	last_ts=int(alpha.get('last_trade_ts',-1));max_ts=last_ts;own=state.own_trades or{};costs=alpha.setdefault('costs',{})
	for product in _R4_CORE_RECYCLE_PRODUCTS:
		for trade in own.get(product,[])or[]:
			try:ts=int(trade.timestamp)
			except(TypeError,ValueError):continue
			if ts<=last_ts:continue
			side=None
			if getattr(trade,'buyer',None)=='SUBMISSION':side='buy'
			elif getattr(trade,'seller',None)=='SUBMISSION':side='sell'
			if side is not None:
				cost=costs.setdefault(product,{'position':0,'avg_price':None});_r4_cost_apply(cost,side,float(trade.price),int(trade.quantity))
			if ts>max_ts:max_ts=ts
	alpha['last_trade_ts']=max_ts
def _r4_update_gate(state,alpha):
	depth=state.order_depths.get('VELVETFRUIT_EXTRACT')
	if depth is None or not depth.buy_orders or not depth.sell_orders:return
	mid=(max(depth.buy_orders)+min(depth.sell_orders))/2.
	if alpha.get('open_mid')is None:alpha['open_mid']=float(mid)
	if not alpha.get('gate_decided',False)and state.timestamp>=_R4_GATE_TS:
		alpha['gate_active']=float(alpha['open_mid'])-float(mid)>=_R4_GATE_DROP;alpha['gate_decided']=True
def _r4_core_recycle_orders(product,state,alpha):
	if product not in _R4_CORE_RECYCLE_PRODUCTS or not alpha.get('gate_active',False)or state.timestamp<_R4_CORE_RECYCLE_START:return[]
	depth=state.order_depths.get(product)
	if depth is None or not depth.buy_orders:return[]
	position=int(state.position.get(product,0));cost=(alpha.get('costs')or{}).get(product,{})
	try:tracked_pos=int(cost.get('position',0))
	except(TypeError,ValueError):tracked_pos=0
	avg=cost.get('avg_price')
	if position<_R4_CORE_RECYCLE_ACTIVE_ABS:return[]
	threshold=float(avg)+_R4_CORE_RECYCLE_TAKE_PROFIT if avg is not None and tracked_pos>=_R4_CORE_RECYCLE_ACTIVE_ABS else float(_R4_CORE_RECYCLE_FALLBACK_SELL[product])
	best_bid=max(depth.buy_orders);bid_volume=depth.buy_orders[best_bid];qty=min(_R4_CORE_RECYCLE_MAX_ORDER,bid_volume,position-_R4_CORE_RECYCLE_FLOOR_ABS)
	if qty>0 and float(best_bid)>=threshold:return[Order(product,best_bid,-qty)]
	return[]
def _velvet_config_for_timestamp(product,schedule,timestamp,alpha):
	cfg=schedule[0][1]
	for start,candidate in schedule:
		if timestamp>=start:cfg=candidate
		else:break
	if product=='VELVETFRUIT_EXTRACT'and alpha.get('gate_active',False)and timestamp>=_R4_GATE_TS:
		cfg=dict(cfg);cfg['buy']=_R4_VELVET_GATE_BUY;cfg['sell']=_R4_VELVET_GATE_SELL
	return cfg
def _velvet_orders_for_product(product,schedule,state,alpha):
	depth=state.order_depths.get(product)
	if depth is None:return[]
	recycle_orders=_r4_core_recycle_orders(product,state,alpha)
	if recycle_orders:return recycle_orders
	cfg=_velvet_config_for_timestamp(product,schedule,state.timestamp,alpha);position=state.position.get(product,0);orders=[]
...
class VelvetScheduleTrader:
	...
	def run(self,state):
		alpha=_r4_load_alpha_state(state.traderData)
		if int(alpha.get('last_seen_ts',-1))>int(state.timestamp):alpha=_r4_default_alpha_state()
		alpha['last_seen_ts']=int(state.timestamp);_r4_process_own_trades(state,alpha);_r4_update_gate(state,alpha)
		...
		for product,schedule in _VELVET_SCHEDULES.items():
			orders=_velvet_orders_for_product(product,schedule,state,alpha)
			if orders:all_orders[product]=all_orders.get(product,[])+orders
		trader_data=_r4_save_alpha_state(trader_data,alpha)
...
# ====================================================================
# R4 COMBO PROBE -- stack_hydhardlong80_60k.
#
...
# ====================================================================
_R4_COMBO_HYD_BASE_TRADER = _R4ComboBaseTrader
...
_R4_COMBO_HYD_TARGET_POS = 120

class _StackHyd120Trader:
    def __init__(self):
    ...
    def is_high_regime_long_active(self, ts: int) -> bool:
        return self._hyd_high_regime and ts < _R4_COMBO_HYD_CONTROL_UNTIL
...
# R4 COMBO v1: stack + HYD target=120 + Mark38 HYD taker_buy fade.
_M38P='HYDROGEL_PACK';_M38W=5_000;_M38C=5_000;_M38S=5;_M38B='Mark 38';_M38T=990_000

class Trader:
	def __init__(self):
		self._inner=_StackHyd120Trader();self._pb=None;self._m38a=0;self._m38f=-10**9;self._seen=set();self._lts=None
	def run(self,state):
		ts=int(state.timestamp)
		if self._lts is not None and ts<self._lts:
			self._pb=None;self._m38a=0;self._m38f=-10**9;self._seen=set()
		self._lts=ts
		for tr in (state.market_trades or {}).get(_M38P,[]) or []:
			b=getattr(tr,'buyer',None);p=int(getattr(tr,'price',0) or 0);q=int(getattr(tr,'quantity',0) or 0);tts=int(getattr(tr,'timestamp',ts) or ts);s=getattr(tr,'seller',None)
			k=(tts,p,q,b,s)
			if k in self._seen:continue
			self._seen.add(k)
			if self._pb is None or p<self._pb[1] or b!=_M38B:continue
			self._m38a=max(self._m38a,ts+_M38W)
		orders,conv,td=self._inner.run(state)
		if ts<self._m38a and ts-self._m38f>=_M38C and ts<_M38T and not self._inner.is_high_regime_long_active(ts) and int(state.position.get(_M38P,0))<=0:
			d=state.order_depths.get(_M38P)
			if d and d.buy_orders:
				bid=int(max(d.buy_orders));ex=list((orders or {}).get(_M38P,[]) or []);es=sum(-int(o.quantity) for o in ex if int(o.quantity)<0)
				if es<_M38S:
					orders=dict(orders or {});orders.setdefault(_M38P,[]).append(Order(_M38P,bid,-(_M38S-max(0,es))));self._m38f=ts
		d=state.order_depths.get(_M38P)
		if d and d.buy_orders and d.sell_orders:self._pb=(int(max(d.buy_orders)),int(min(d.sell_orders)))
		return orders,conv,td
