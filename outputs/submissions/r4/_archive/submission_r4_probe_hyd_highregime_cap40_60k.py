...
# ====================================================================
# R4 HYDROGEL PROBE — high-regime short cap.
#
# Official calibration question:
#   If HYD is still above 10020 in the 20k-30k discovery window, does
#   delaying most short inventory until later high-regime prices improve
#   the official 100k slice?
#
# Mechanism:
#   - Detect high regime from the observed book, not from exact timestamps.
#   - While active and before 60k, do not let HYD position go below -40.
#   - Leave the existing 995k final flatten wrapper intact for 1M replay.
# ====================================================================
_R4_HYD_HIGHREGIME_BASE_TRADER = Trader
_HYD_HR_TRIGGER_START = 20_000
_HYD_HR_TRIGGER_END = 30_000
_HYD_HR_TRIGGER_MID = 10_020.0
_HYD_HR_CONTROL_UNTIL = 60_000
_HYD_HR_SHORT_CAP = -40

class Trader:
    def __init__(self):
        self._inner = _R4_HYD_HIGHREGIME_BASE_TRADER()
        self._hyd_high_regime = False
    def run(self, state):
        orders, conversions, trader_data = self._inner.run(state)
        self._observe_hyd_regime(state)
        if self._hyd_high_regime and state.timestamp < _HYD_HR_CONTROL_UNTIL:
            orders = self._apply_hyd_short_cap(state, orders, _HYD_HR_SHORT_CAP)
        return orders, conversions, trader_data
    def _observe_hyd_regime(self, state):
        if self._hyd_high_regime:
            return
        if state.timestamp < _HYD_HR_TRIGGER_START or state.timestamp > _HYD_HR_TRIGGER_END:
            return
        depth = state.order_depths.get('HYDROGEL_PACK')
        if depth is None or not depth.buy_orders or not depth.sell_orders:
            return
        mid = (max(depth.buy_orders.keys()) + min(depth.sell_orders.keys())) / 2
        if mid >= _HYD_HR_TRIGGER_MID:
            self._hyd_high_regime = True
    def _apply_hyd_short_cap(self, state, orders, short_cap):
        orders = dict(orders or {})
        depth = state.order_depths.get('HYDROGEL_PACK')
        pos = int(state.position.get('HYDROGEL_PACK', 0))
