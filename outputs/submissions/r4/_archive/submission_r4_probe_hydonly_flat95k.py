...
# ====================================================================
# R4 DIAGNOSTIC — HYDROGEL ONLY.
# Override Trader to emit ONLY HYDROGEL orders. Velvet/voucher orders
# returned by the inner R3 trader are dropped. Used for official-
# simulator PnL attribution: comparing this to the full submission tells
# us how much of total PnL comes from Hydrogel vs Velvet/options.
# ====================================================================
_R3_BASE_TRADER_HYDONLY = Trader

class Trader:
    def __init__(self):
        self._inner = _R3_BASE_TRADER_HYDONLY()
    def run(self, state):
        orders, conversions, trader_data = self._inner.run(state)
        if orders:
            orders = {p: v for p, v in orders.items() if p == 'HYDROGEL_PACK'}
        return orders, conversions, trader_data

# ====================================================================
# R4 HYDROGEL OFFICIAL-CALIBRATION PROBE — HYD-only flat at 95k.
#
# Purpose:
#   Test whether the official 100k HYDROGEL result contains realized
#   round-trip edge or is mostly a terminal-mark short-position artifact.
#
# Interpretation:
#   - This is NOT a final 1M candidate.
#   - Upload only as a calibration experiment.
#   - If official HYD PnL survives this forced early flatten, the official
#     slice has realized HYD path edge. If it collapses, the current official
#     HYD score is mostly terminal exposure.
# ====================================================================
_R4_HYDONLY_BASE_TRADER = Trader
_HYD_OFFICIAL_PROBE_FLATTEN_TS = 95_000

class Trader:
    def __init__(self):
        self._inner = _R4_HYDONLY_BASE_TRADER()
    def run(self, state):
        orders, conversions, trader_data = self._inner.run(state)
        orders = dict(orders or {})
        if state.timestamp >= _HYD_OFFICIAL_PROBE_FLATTEN_TS:
            ...
            if pos != 0:
                ...
                if depth is not None:
                    if pos > 0 and depth.buy_orders:
                        ...
                        orders['HYDROGEL_PACK'] = [Order('HYDROGEL_PACK', int(best_bid), -pos)]
                    elif pos < 0 and depth.sell_orders:
                        best_ask = min(depth.sell_orders.keys())
                        orders['HYDROGEL_PACK'] = [Order('HYDROGEL_PACK', int(best_ask), -pos)]
        return orders, conversions, trader_data
