...
# ====================================================================
# R4 MARK55 PASSIVE PROBE — Mark67-conditioned inside VELVET bid.
#
# Diagnostic upload, not a recommended final yet.
#
# Hypothesis from Round 4 counterparty audit:
#   recent Mark 67 VELVET buy-taker flow raises the short-horizon
#   probability of Mark 55 VELVET sell-taker flow. The executable response is
#   to post a small non-crossing bid one tick inside the current best bid,
#   letting a future seller hit us instead of paying the ask.
#
# Local replay undercredits this mechanism because historical trades print at
# the old book, not the book after our inside quote. Official simulator upload
# is the calibration experiment.
# ====================================================================
_MARK55_BASE_TRADER = HydFlatTrader
_MARK55_PROBE_PRODUCT = 'VELVETFRUIT_EXTRACT'
_MARK55_PROBE_SIZE = 5
_MARK55_PROBE_TARGET_MAX_POS = -150
_MARK55_PROBE_WINDOW = 30_000
_MARK55_PROBE_COUNT_THRESHOLD = 3

class _Mark55RollingCounter:
    ...
    def prune(self, timestamp):
        ...
        while self.events and self.events[0][0] < cutoff:
            ...
            self.total_qty -= qty
    def add(self, timestamp, quantity):
...
class Mark55Q5Trader:
    def __init__(self):
        self._inner = _MARK55_BASE_TRADER()
        self._mark67_buy = _Mark55RollingCounter(_MARK55_PROBE_WINDOW)
    ...
    def _ingest(self, state):
        ...
        for trade in (state.market_trades or {}).get(_MARK55_PROBE_PRODUCT, []) or []:
            if getattr(trade, 'buyer', None) == 'Mark 67':
                self._mark67_buy.add(ts, int(getattr(trade, 'quantity', 0) or 0))
    def _active(self):
        return self._mark67_buy.count >= _MARK55_PROBE_COUNT_THRESHOLD
    def _append_probe(self, orders, state):
        depth = state.order_depths.get(_MARK55_PROBE_PRODUCT)
        if depth is None or not depth.buy_orders or not depth.sell_orders:
            return
        existing = list((orders or {}).get(_MARK55_PROBE_PRODUCT, []) or [])
        if any(int(order.quantity) > 0 for order in existing):
            return
        pos = int(state.position.get(_MARK55_PROBE_PRODUCT, 0))
        buy_capacity = _MARK55_PROBE_TARGET_MAX_POS - pos
        if buy_capacity <= 0:
            return
        bid = int(max(depth.buy_orders))
        ask = int(min(depth.sell_orders))
        price = bid + 1 if bid + 1 < ask else bid
        if price >= ask:
            return
        qty = min(_MARK55_PROBE_SIZE, buy_capacity)
        if qty > 0:
            orders.setdefault(_MARK55_PROBE_PRODUCT, []).append(Order(_MARK55_PROBE_PRODUCT, price, qty))
    def run(self, state):
        self._ingest(state)
        orders, conversions, trader_data = self._inner.run(state)
        orders = dict(orders or {})
        if self._active():
            self._append_probe(orders, state)
        return orders, conversions, trader_data
...
# ====================================================================
_M55_SINGLE_BASE_TRADER = Mark55Q5Trader
