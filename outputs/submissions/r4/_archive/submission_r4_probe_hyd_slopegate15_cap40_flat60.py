...
_HYDROGEL_PUBLICGUARD_TRADER=Trader
...
_VELVET_SCHEDULES={
	...
	'VEV_5500':((0,{'limit':300,'max_order':40,'buy':7,'sell':8}),),
}
...
# ====================================================================
# R4 HYDROGEL PROBE - slope-gated cap40 flat60.
#
# Robustness question:
#   Can we keep most of the official high-regime inventory/release edge
#   without blindly fitting the public day-3/official 100k prefix?
#
# Mechanism:
#   - If HYD mid >= 10020 during 20k-30k, enter suspect high-regime mode.
#   - Until 40k, cap short inventory at -40 rather than forcing flat/long.
#   - At 40k, require the 20k->40k mid slope to be at least +15.
#   - If confirmed, force flat until bid >= 10048 at 60k, then release.
#   - If rejected/missed, abort and flatten HYD exposure.
# ====================================================================
_R4_HYD_SLOPE_BASE_TRADER = Trader
_HYD_SLOPE_TRIGGER_START = 20_000
_HYD_SLOPE_TRIGGER_END = 30_000
_HYD_SLOPE_TRIGGER_MID = 10_020.0
_HYD_SLOPE_START_TS = 20_000
_HYD_SLOPE_GATE_TS = 40_000
_HYD_SLOPE_THRESHOLD = 15.0
_HYD_SLOPE_CONFIRM_TS = 60_000
_HYD_SLOPE_CONFIRM_BID = 10_048
_HYD_SLOPE_SHORT_CAP = -40
_HYD_SLOPE_TARGET_POS = 0

class Trader:
    def __init__(self):
        self._inner = _R4_HYD_SLOPE_BASE_TRADER()
        self._hyd_triggered = False
        self._hyd_released = False
        self._hyd_aborted = False
        self._hyd_slope_mid = None
        self._hyd_slope_passed = False
    ...
    def run(self, state):
        ...
        if self._hyd_triggered and not self._hyd_released:
            if self._hyd_slope_passed:
                orders = self._hard_target_hyd(state, orders, _HYD_SLOPE_TARGET_POS)
            else:
                orders = self._apply_short_cap(state, orders, _HYD_SLOPE_SHORT_CAP)
        return orders, conversions, trader_data

    def _observe_slope_start(self, state):
        if self._hyd_slope_mid is not None or state.timestamp < _HYD_SLOPE_START_TS:
            return
    ...
    def _observe_trigger(self, state):
        if self._hyd_triggered:
            return
        if state.timestamp < _HYD_SLOPE_TRIGGER_START or state.timestamp > _HYD_SLOPE_TRIGGER_END:
            return
        mid = self._mid(state)
        if mid is not None and mid >= _HYD_SLOPE_TRIGGER_MID:
            self._hyd_triggered = True

    def _observe_slope_gate(self, state):
        if (
            not self._hyd_triggered
            or self._hyd_slope_passed
            or self._hyd_aborted
            or state.timestamp < _HYD_SLOPE_GATE_TS
        ):
            return
        mid = self._mid(state)
        if (
            mid is not None
            and self._hyd_slope_mid is not None
            and mid - self._hyd_slope_mid >= _HYD_SLOPE_THRESHOLD
        ):
            self._hyd_slope_passed = True
        else:
    ...
    def _observe_release_or_abort(self, state):
        if not self._hyd_triggered or self._hyd_released or self._hyd_aborted:
            return
        if not self._hyd_slope_passed:
            return
        if state.timestamp >= _HYD_SLOPE_CONFIRM_TS:
            bid = self._best_bid(state)
            if bid is not None and bid >= _HYD_SLOPE_CONFIRM_BID:
    ...
    def _apply_short_cap(self, state, orders, short_cap):
        orders = dict(orders or {})
        depth = state.order_depths.get('HYDROGEL_PACK')
        pos = int(state.position.get('HYDROGEL_PACK', 0))
        if depth is not None and pos < short_cap and depth.sell_orders:
            best_ask = min(depth.sell_orders.keys())
            orders['HYDROGEL_PACK'] = [Order('HYDROGEL_PACK', int(best_ask), int(short_cap - pos))]
            return orders
        filtered = []
        projected = pos
        for order in list(orders.get('HYDROGEL_PACK', [])):
            qty = int(order.quantity)
            if qty < 0:
                max_sell = max(0, projected - short_cap)
                allowed = min(-qty, max_sell)
                if allowed > 0:
                    filtered.append(Order('HYDROGEL_PACK', int(order.price), -allowed))
                    projected -= allowed
            else:
                filtered.append(order)
                projected += qty
        orders['HYDROGEL_PACK'] = filtered
        return orders
