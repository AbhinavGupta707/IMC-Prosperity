...
class _R4StackHydAbortBaseTrader:
    def __init__(self):
...
# high-regime overlay.
# ====================================================================
_R4_HYD_ABORT_BASE_TRADER = _R4StackHydAbortBaseTrader
...
class _AbortGateHyd80Trader:
    ...
    def is_high_regime_long_active(self, ts: int) -> bool:
        return self._hyd_triggered and not self._hyd_released and not self._hyd_aborted
...
# R4 final risk-guarded Mark38 overlay:
#   - keeps the abortgate18_long80_60 HYD spine;
#   - when Mark 38 buys HYD at/through the previous ask, lightly fades
#     with one 5-lot best-bid sell while not in the protected long window.
_M38P = 'HYDROGEL_PACK'
_M38W = 5_000
_M38C = 5_000
_M38S = 5
...
class Trader:
    def __init__(self):
        self._inner = _AbortGateHyd80Trader()
        self._pb = None
        self._m38a = 0
        self._m38f = -10**9
        self._seen = set()
    ...
    def run(self, state):
        ...
        for tr in (state.market_trades or {}).get(_M38P, []) or []:
            ...
            price = int(getattr(tr, 'price', 0) or 0)
            qty = int(getattr(tr, 'quantity', 0) or 0)
            tts = int(getattr(tr, 'timestamp', ts) or ts)
            seller = getattr(tr, 'seller', None)
            key = (tts, price, qty, buyer, seller)
            if key in self._seen:
                continue
            self._seen.add(key)
            if self._pb is None or price < self._pb[1] or buyer != _M38B:
                continue
        ...
        if (
            ts < self._m38a
            and ts - self._m38f >= _M38C
            and ts < _M38T
            and not self._inner.is_high_regime_long_active(ts)
            and int(state.position.get(_M38P, 0)) <= 0
        ):
            depth = state.order_depths.get(_M38P)
            if depth and depth.buy_orders:
                bid = int(max(depth.buy_orders))
                existing = list((orders or {}).get(_M38P, []) or [])
                existing_sell = sum(-int(o.quantity) for o in existing if int(o.quantity) < 0)
                if existing_sell < _M38S:
                    orders = dict(orders or {})
                    orders.setdefault(_M38P, []).append(
                        Order(_M38P, bid, -(_M38S - max(0, existing_sell)))
                    )
                    self._m38f = ts

        depth = state.order_depths.get(_M38P)
        if depth and depth.buy_orders and depth.sell_orders:
            self._pb = (int(max(depth.buy_orders)), int(min(depth.sell_orders)))
        return orders, conversions, trader_data
