...
# ====================================================================
# R4 MARK22-SELL-GATED VEV_5000/VEV_5100 RECYCLE PROBE — TREATMENT ARM.
#
# Hypothesis: when Mark22 has been actively selling vouchers or VELVET
# in the recent past, our short-horizon VEV_5000/5100 recycle-sells
# achieve a better mark than firing at random.
#
# Treatment arm: gate is "Mark 22 was the seller in any VEV_*/VELVET
# trade observed in market_trades within the last 5,000 ticks". When
# active and we hold a profitable long, append a single-shot extra
# sell of size 5 at best_bid for VEV_5000 and VEV_5100, subject to a
# 5,000-tick cooldown between wrapper fires.
#
# Pair this with submission_r4_probe_m22sell_recycle_control.py, which
# replaces the Mark22 gate with a fixed 10,000-tick time cadence.
# Compare official PnL deltas to test whether Mark22 identity adds
# alpha beyond regime/timing.
#
# Kill criteria:
#   * Treatment loses >= -200 vs validated baseline officially -> abort.
#   * |treatment - control| < 100 -> Mark22 ID has no incremental edge.
#   * Treatment > control by >= 150 with same sign -> Mark22 ID is real.
# ====================================================================
_M22T_PRODUCTS = ('VEV_5000', 'VEV_5100')
_M22T_PRODUCT_THRESH = {'VEV_5000': 268, 'VEV_5100': 177}
_M22T_PRODUCT_MIN_POS = {'VEV_5000': 100, 'VEV_5100': 100}
_M22T_FIRE_SIZE = 5
_M22T_GATE_WINDOW = 5_000
_M22T_FIRE_COOLDOWN = 5_000
_M22T_GATE_SYMBOLS = (
    'VELVETFRUIT_EXTRACT',
    'VEV_4000', 'VEV_4500', 'VEV_5000', 'VEV_5100', 'VEV_5200',
    'VEV_5300', 'VEV_5400', 'VEV_5500', 'VEV_6000', 'VEV_6500',
)
_M22T_TARGET_MARK = 'Mark 22'

class Trader:
    def __init__(self):
        self._inner = _BaseTrader()
    ...
    def _gate_active(self, ts):
        if self._gate_recent_ts <= 0:
            return False
        return (ts - self._gate_recent_ts) <= _M22T_GATE_WINDOW

    def _maybe_fire(self, orders, state):
        ts = int(state.timestamp)
        if not self._gate_active(ts):
            return orders
        if ts - self._last_fire_ts < _M22T_FIRE_COOLDOWN:
            return orders
        fired_any = False
        new_orders = dict(orders or {})
        for product in _M22T_PRODUCTS:
            depth = state.order_depths.get(product)
            if depth is None or not depth.buy_orders:
                continue
            pos = int(state.position.get(product, 0))
            if pos < _M22T_PRODUCT_MIN_POS[product]:
                continue
            best_bid = int(max(depth.buy_orders))
            if best_bid < _M22T_PRODUCT_THRESH[product]:
                continue
            existing = list(new_orders.get(product, []) or [])
            already_selling = sum(
                -int(o.quantity) for o in existing if int(o.quantity) < 0
            )
            qty = min(_M22T_FIRE_SIZE, pos)
            if qty <= 0 or already_selling >= qty:
                continue
