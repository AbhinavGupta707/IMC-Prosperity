...
# R4 TIME-CADENCE CONTROL: stack + HYD target=120 + fade-shorts on time cadence
# (no Mark filter). Fires every 5k ticks when HYD pos<=0 and not in long regime.
# This is the critical test of "is the Mark 38 fade alpha really Mark identity,
# or just generic 'add-shorts-when-already-short'?"
_TC_P='HYDROGEL_PACK';_TC_PERIOD=5_000;_TC_S=5;_TC_T=990_000

class Trader:
	def __init__(self):
		self._inner=_StackHyd120Trader();self._tcf=-10**9;self._lts=None
	def run(self,state):
		...
		# Fire on time cadence (every 5k) when conditions match
		if ts-self._tcf>=_TC_PERIOD and ts<_TC_T and not self._inner.is_high_regime_long_active(ts) and int(state.position.get(_TC_P,0))<=0:
			d=state.order_depths.get(_TC_P)
			if d and d.buy_orders:
				bid=int(max(d.buy_orders));ex=list((orders or {}).get(_TC_P,[]) or []);es=sum(-int(o.quantity) for o in ex if int(o.quantity)<0)
				if es<_TC_S:
					orders=dict(orders or {});orders.setdefault(_TC_P,[]).append(Order(_TC_P,bid,-(_TC_S-max(0,es))));self._tcf=ts
