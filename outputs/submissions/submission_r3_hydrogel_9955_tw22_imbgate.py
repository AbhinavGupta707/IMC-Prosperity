...
_HYDROGEL_PARAMS=SSTParams(take_width=22.,clear_threshold=1.,clear_width=2.,default_edge=3.,disregard_edge=1.,join_edge=3.,default_quote_size=20,max_taker_size=200,prevent_adverse=False,quote_inside_wall=False,wall_min_volume=10)
def hydrogel_orders(snapshot,position,timestamp,params=_HYDROGEL_PARAMS,skew_strength=1.,cycle_state=None):
	...
	rebound_exit=_rebound_long_exit_order(snapshot,position,cycle_state)
...
def _adaptive_params(snapshot,position,params):
	if not _HYDROGEL_ENABLE_IMBALANCE_GATE:return params
	if position>0 or snapshot.mid is None:return params
	mid=float(snapshot.mid)
	if mid<_HYDROGEL_MEAN+_HYDROGEL_IMBALANCE_ZONE:return params
